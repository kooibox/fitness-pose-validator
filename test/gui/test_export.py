...
def test_export_session(session_id: int, compress: bool = False):
    ...
    try:
        ...
        # 导出为JSON
        output_path = exporter.export_to_json(session_id=session_id, compress=compress)
...
def list_sessions(limit: int = 10):
    ...
    for session in sessions:
        print(
            f"{session.id:<8} {session.start_time:<26} {session.total_squats:<10} {session.total_frames:<10}"
        )
...
示例:
  ...
  python test_export.py --upload http://localhost:8000/api/upload
    ...
    parser.add_argument("--all", action="store_true", help="导出所有会话")

    parser.add_argument("-l", "--list", action="store_true", help="列出所有会话")

    parser.add_argument("--upload", type=str, metavar="URL", help="上传到服务器URL")

    parser.add_argument("--token", type=str, help="认证令牌")

    parser.add_argument("--limit", type=int, default=10, help="限制会话数量 (默认: 10)")
