...
def test_upload_workflow():
    ...
    config_file = project_root / "data" / "gui_settings.json"
    ...
    print(
        f"✓ 数据压缩: {len(json_bytes)} → {len(compressed)} bytes ({len(compressed) / len(json_bytes) * 100:.1f}%)"
    )
    ...
    request = urllib.request.Request(
        server_url, data=compressed, headers=headers, method="POST"
    )
